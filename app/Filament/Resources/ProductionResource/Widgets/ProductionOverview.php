...
//            Stat::make('Produzione fatturata', '€'.$this->getPageTableQuery()->where('imponibile','>',0,'and',)->sum('imponibile'))
            Stat::make('Produzione fatturata', '€'.Production::query()->where('imponibile','>',0,'and')->where('status','fatturato')->sum('imponibile'))
//                ->description('32k increase')
                ->descriptionIcon('heroicon-m-arrow-trending-up')
                ->color('success'),
            Stat::make('Produzione stimata', '€'.Production::query()->where('imponibile','>',0,'and')->where('status','stimato')->sum('imponibile'))
//                ->description('7% increase')
                ->descriptionIcon('heroicon-m-arrow-trending-down')
                ->color('estimated'),
            Stat::make('Produzione contabilizzata e non fatturata', '€'.Production::query()->where('imponibile','>',0,'and')->where('status','contabilizzato e non ft')->sum('imponibile'))
